...
def login_appinspect(username, password, verify_ssl=True):
    login_url = "https://api.splunk.com/2.0/rest/login/splunk"

    try:
        response = requests.get(
            login_url, auth=HTTPBasicAuth(username, password), verify=verify_ssl
        )
...
def submit_appinspect(session, token, app, verify_ssl=True):
    appinspect_headers = {
    ...
    try:
        response = session.post(
            validate_url, headers=appinspect_headers, files=files, verify=verify_ssl
        )
        if response.status_code not in (200, 201, 204):
            logging.error(
...
def verify_appinspect(session, token, request_id, verify_ssl=True):
    appinspect_headers = {
        "Authorization": "bearer %s" % token,
    }
    ...
    try:
        response = session.get(
            validate_url, headers=appinspect_headers, verify=verify_ssl
        )
        if response.status_code not in (200, 201, 204):
...
def download_htmlreport_appinspect(session, token, request_id, verify_ssl=True):
    appinspect_headers = {
        "Authorization": "bearer %s" % token,
...
# download an Appinspect vetting report
def download_jsonreport_appinspect(session, token, request_id, verify_ssl=True):
    appinspect_headers = {
        "Authorization": "bearer %s" % token,
